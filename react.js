...
import { ChainId, Fetcher, Route, Trade, TokenAmount, TradeType, Percent, JSBI, Token, Pair, } from "@dynamic-amm/sdk";

const App = () => {
  //**** create provider for mumbai */
  const alchemyKey: string = process.env.REACT_APP_ALCHEMY_KEY!.toString().trim();
  const provider = new providers.AlchemyProvider( 80001, alchemyKey ) as providers.AlchemyProvider;
  const chainId = ChainId.MUMBAI as number;

  //**** transaction settings */
  const addrFactory: string = utils.getAddress( "0x7900309d0b1c8D3d665Ae40e712E8ba4FC4F5453" ); // dmm factory contract on mumbai
  const addrTokenA: string = utils.getAddress( "0x001b3b4d0f3714ca98ba10f6042daebf0b1b7b6f" ); // dai on mumbai
  const addrTokenB: string = utils.getAddress( "0x2058a9d7613eee744279e3856ef0eada5fcbaa7e" ); // usdc on mumbai

  //**** async function to swap */
  const getDatas = async () => {
    // define tokens for swap
    const tokenA: Token = await Fetcher.fetchTokenData( chainId, addrTokenA, provider, "DAI", "DAI stablecoin"); // dai token object
    const tokenB: Token = await Fetcher.fetchTokenData( chainId, addrTokenB, provider, "USDC", "USDC stablecoin" ); // usdc token object
    
    // define Pair[] object [dai, usdc]
    const pair: Pair[] = await Fetcher.fetchPairData( tokenA, tokenB, addrFactory, provider );
    ...
    const amountInValue: BigNumber = utils.parseUnits( "100.0", route.path[0].decimals);
    const amountIn: TokenAmount = new TokenAmount( tokenA, amountInValue.toBigInt());

    // define trade object
    const trade = new Trade(route, amountIn, TradeType.EXACT_INPUT) as Trade;
    // define slippage settings = 0.5%
    const slippageTolerance: Percent = new Percent("50", "10000");
    // compute final minimum output amount
    const amountOutMin: JSBI = trade.minimumAmountOut(slippageTolerance).raw;
    // needs to be converted to e.g. hex
    const amountOutMinHex: string = BigNumber.from( amountOutMin.toString() ).toHexString();
    ...
    const to: string = utils.getAddress(
      "0x..."
    );
    ...
    const signer = Wallet.fromMnemonic( process.env.REACT_APP_MNEMONIC!, process.env.REACT_APP_ACCOUNT_PATH!) as Wallet;
    ...
    const DmmRouterInterface = new utils.Interface( DmmRouter) as utils.Interface;
    ...
    console.log(`Transaction was mined in block ${receiptTxSwap.blockNumber}`);
  };
